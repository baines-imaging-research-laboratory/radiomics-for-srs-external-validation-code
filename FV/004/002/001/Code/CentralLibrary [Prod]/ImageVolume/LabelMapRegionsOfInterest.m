...
%     end
    ...
    methods (Access = public)
        ...
        function dROINumber = GetRegionOfInterestNumberByRegionOfInterestName(obj, sRegionOfInterestName)
            arguments
                obj LabelMapRegionsOfInterest
                sRegionOfInterestName (1,1) string
            end
            
            dROINumber = find(obj.GetRegionsOfInterestNames() == sRegionOfInterestName);
            
            if ~isscalar(dROINumber)
                error(...
                    'LabelMapRegionsOfInterest:GetRegionOfInterestNumberByRegionOfInterestName:InvalidRegionOfInterestName',...
                    'Region of interest name needs to be in the region of interest name list and unique.');
            end
        end
        ...
        function LoadVolumeData(obj)
            if isempty(obj.m3uiLabelMaps)
                if ~isempty(obj.chMatFilePath)
                    [m3uiLabelMaps, oObjFromMatFile] = FileIOUtils.LoadMatFile(...
                        obj.chMatFilePath,...
                        obj.chLabelMapsMatFileVarName,...
                        obj.chObjMatFileVarName);
                    
                    obj.dCurrentAppliedImagingObjectTransform = oObjFromMatFile.dCurrentAppliedImagingObjectTransform;
    ...
    methods (Access = protected)
        ...
        function MustBeValidMatFilePath_ChildClass(obj, chMatFilePath)
            
            % local call
            if ~any(obj.vsVariablesInMatFile == LabelMapRegionsOfInterest.chLabelMapsMatFileVarName)
                error(...
                    'LabelMapRegionsOfInterest:MustBeValidMatFilePath_ChildClass:InvalidMatFile',...
