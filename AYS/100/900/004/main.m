...
sExpCode_Clinical_LRCP = "EXP-100-500-001";
sExpCode_Radiomic_LRCP = "EXP-100-500-002";
sExpCode_ClinicalAndRadiomic_LRCP = "EXP-100-500-003";

oLBL_VUMC = ExperimentManager.Load('LBL-101-001-001');
oLabelledFeatureValues_VUMC = oLBL_VUMC.GetLabelledFeatureValues();

dNumPositives_VUMC = sum(oLabelledFeatureValues_VUMC.GetLabels() == oLabelledFeatureValues_VUMC.GetPositiveLabel());
dNumNegatives_VUMC = sum(oLabelledFeatureValues_VUMC.GetLabels() == oLabelledFeatureValues_VUMC.GetNegativeLabel());

oLBL_LRCP = ExperimentManager.Load('LBL-002-002-001A');
oLabelledFeatureValues_LRCP = oLBL_LRCP.GetLabelledFeatureValues();

dNumPositives_LRCP = sum(oLabelledFeatureValues_LRCP.GetLabels() == oLabelledFeatureValues_LRCP.GetPositiveLabel());
dNumNegatives_LRCP = sum(oLabelledFeatureValues_LRCP.GetLabels() == oLabelledFeatureValues_LRCP.GetNegativeLabel());

dNumPositives = dNumPositives_VUMC + dNumPositives_LRCP;
dNumNegatives = dNumNegatives_VUMC + dNumNegatives_LRCP;

[m2dXAndCI_ClinicalOnly, m2dYAndCI_ClinicalOnly, vdAUCAndCI_ClinicalOnly, vdMCRAndCI_ClinicalOnly, vdFNRAndCI_ClinicalOnly, vdFPRAndCI_ClinicalOnly, dOptimalThresholdPointIndex_ClinicalOnly, dAUC_0Point632PlusPerBootstrap_ClinicalOnly] = GenerateROCAndCIMetrics(sExpCode_Clinical_LRCP, dNumPositives, dNumNegatives);
